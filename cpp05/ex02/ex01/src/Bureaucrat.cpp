...
void	Bureaucrat::signForm( AForm &form ) const
{
	...
	{
		form.beSigned(*this);
		std::cout << GREEN << "✅ Bureaucrat " << BOLD_WHITE << _name << RESET << GREEN
			<< " signed form " << BOLD_WHITE << form.getName() << RESET
			<< GREEN << " with grade " << BOLD_YELLOW << _grade << RESET << std::endl;
	}
	catch (const std::exception &e)
	{
		std::cout << RED << "❌ Bureaucrat " << BOLD_WHITE << _name << RESET << RED
			<< " could not sign form " << BOLD_WHITE << form.getName() << RESET
...
void	Bureaucrat::executeForm( const AForm &form ) const
{
	try
	{
		form.execute(*this);
		std::cout << GREEN << "✅ Bureaucrat " << BOLD_WHITE << _name << RESET << GREEN
			<< " executed form " << BOLD_WHITE << form.getName() << RESET
			<< GREEN << " with grade " << BOLD_YELLOW << _grade << RESET << std::endl;
	}
	catch (const std::exception &e)
	{
		std::cout << RED << "❌ Bureaucrat " << BOLD_WHITE << _name << RESET << RED
			<< " could not execute form " << BOLD_WHITE << form.getName() << RESET
			<< RED << " because: " << e.what() << RESET << std::endl;
		return;
	}
}
